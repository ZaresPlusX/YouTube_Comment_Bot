...
function onCommented(message) {
    chrome.storage.local.get('watchurls', function (result_urls) {
        ...
        chrome.storage.local.get('tabid', function (result_tabid) {
            chrome.tabs.update(result_tabid.tabid, {
                url: 'https://www.youtube.com' + urls[0]
            }, function (tab) {
        ...
        if (urls.length == 1) {
            ...
            chrome.storage.local.get('tabid', function (result_tabid) {
                chrome.storage.local.get('watchurls', function (result_searchurls) {
                    chrome.tabs.update(result_tabid.tabid, {
                        url: 'https://www.youtube.com' + result_searchurls.watchurls[0]
                    }, function (tab) {});
                });
            });
            return;
...
function storeCommentInfo(message) {
    ...
    chrome.storage.local.set({
        'isInstant': message.data.isInstant
    });
    chrome.storage.local.set({
    ...
    chrome.storage.local.set({
        'comment_cnt': message.data.comment_cnt
    });
    console.log(message);
}
